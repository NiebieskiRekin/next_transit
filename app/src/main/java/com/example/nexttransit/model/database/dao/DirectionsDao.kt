...
interface DirectionsDao {
    ...
    SELECT 
        ...
        e2.eventId as second_event_eventId,
        e2.calendarId as second_event_calendarId
    FROM directionsquerycrossref d JOIN event e1 ON d.firstEventId = e1.id JOIN event e2 ON d.secondEventId = e2.id ORDER BY e1.endDateTime ASC, e2.startDateTime ASC
    """)
    ...
    JOIN event e1 ON d.firstEventId = e1.id
    JOIN event e2 ON d.secondEventId = e2.id
    WHERE (e1.startDateTime >= :startTime AND e1.endDateTime <= :endTime)  ORDER BY e1.startDateTime ASC """)
    ...
    JOIN event e1 ON d.firstEventId = e1.id 
    JOIN event e2 ON d.secondEventId = e2.id
    WHERE (:currentTime <= e1.endDateTime+:delta AND :currentTime >= e1.startDateTime-:delta) ORDER BY e1.startDateTime ASC LIMIT 1
    """)
